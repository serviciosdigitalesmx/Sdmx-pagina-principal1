...
HOME = Path.home()
DEFAULT_REPO = HOME / "dev" / "hotel-pms"
if os.environ.get("RALPH_HOTEL_REPO"):
    REPO = Path(os.environ["RALPH_HOTEL_REPO"]).expanduser().resolve()
else:
    cwd_probe = subprocess.run(["git", "rev-parse", "--show-toplevel"], text=True, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
    if cwd_probe.returncode == 0 and cwd_probe.stdout.strip():
        REPO = Path(cwd_probe.stdout.strip()).resolve()
    elif (HOME / "Desktop" / "FIXI" / ".git").exists():
        REPO = (HOME / "Desktop" / "FIXI").resolve()
    else:
...
RESULTS = SUP / "results"
LOGS = SUP / "logs"
BACKUP_ROOT = HOME / ".ralph-fixi" / "backups"
STATE_FILE = SUP / "state.json"
GOAL_FILE = SUP / "goal.txt"
POLICY_FILE = SUP / "policy.json"
ACTIVE_FILE = CTRL / "ACTIVE_ASPEC"
DAEMON_PID = HOME / ".ralph-fixi" / "daemon.pid"
DAEMON_LOG = HOME / ".ralph-fixi" / "daemon.log"
...
RALPH = HOME / "dev" / "tools" / "ralph-yeine" / "bin" / "ralph"

EXCLUDE_PARTS = {
    ".git", ".ralph-add", "node_modules", ".pnpm-store", ".turbo", ".cache", ".eslintcache", "build", "dist", ".gradle",
    "target", "coverage", ".next", "__pycache__", ".idea", ".vscode", "tsconfig.tsbuildinfo"
}
SAFE_GENERATED_PREFIXES = (
    "frontend/node_modules/", "frontend/dist/", ".pnpm-store/", ".turbo/", ".cache/", ".eslintcache", ".ralph-add/",
)
...
def plan_next(goal: str, report: Path | None, current_meta: dict) -> dict:
    ...
    prompt = f"""You are the ADD planning supervisor powered by Codex Fast Engine for Fixi.
...
def cmd_plan(args):
    ...
    prompt = f"""You are ADD in PLAN-ONLY mode for the real Fixi repository.

GOAL:
{goal}

Build a complete production-readiness DAG before any execution. Inspect the repository,
docs/canonical/, docs/specs/, AGENTS.md, existing .ralph-add specs and reports. Return JSON ONLY:
{{
  "summary": "...",
...
- This is planning only: do not execute workers, tests, migrations, deploys, or writes outside
  .ralph-add/ADD/PLAN/ and .ralph-add/ADD/specs/.
...
"""
    ...
    atomic_write(PLAN_DIR / f"FIXI-PRODUCTION-MAP-{now()}.md", str(plan.get("master_plan", "")))
    created = []
    ...
    print(f"Master plan: {(PLAN_DIR).relative_to(REPO)}/FIXI-PRODUCTION-MAP-*.md")
    print("Ralph workers were not executed.")

def allowed_change(path: str, allowed: List[str]) -> bool:
    if path.startswith(".ralph-add/"):
        return True
...
Read:
.ralph-add/RALPH/BASE_PROMPT.md
.ralph-add/PROJECT/PROJECT_CONTEXT.md
.ralph-add/PROJECT/INVARIANTS.md
.ralph-add/PROJECT/COMMAND_POLICY.md
...
def cmd_status(_args):
    ensure_control_plane()
    active = get_active_spec()
    print("Ralph Fixi Supervisor v10 (Codex Hypervelocity Engine)")
    print(f"Repo: {REPO}")
...
def cmd_start(args):
    ...
    if not goal:
        die("Goal cannot be empty")
    HOME.joinpath(".ralph-fixi").mkdir(parents=True, exist_ok=True)
    if DAEMON_PID.exists():
        try:
            pid = int(DAEMON_PID.read_text().strip())
            os.kill(pid, 0)
    ...
    atomic_write(GOAL_FILE, goal + "\n")
    print(f"Ralph Fixi Codex-Fast started: pid={p.pid}")
...
HV_SANDBOX_ROOT = HOME / ".ralph-fixi" / "hypervelocity-worktrees"
...
def _hv_prepare_sandbox(spec_id: str) -> Path:
    ...
    excludes = [
        "--exclude=.git", "--exclude=.ralph-add", "--exclude=node_modules", "--exclude=.pnpm-store", "--exclude=.turbo", "--exclude=.cache", "--exclude=.eslintcache",
...
Do not inspect or modify any other Fixi checkout, including /Users/usuario/dev/fixi-production-20260824.
"""

    if meta["mode"] == "READ_ONLY":
        prompt = f"""Execute this Fixi ADD A.SPEC in READ-ONLY mode.
...
def _hv_blocker_key(obj: dict) -> str:
    ...
    text = re.sub(r'\b(?:approval|fixi|ai)-\d+\b', ' ', text)
    ...
    title = re.sub(r'\b(?:approval|fixi|ai)-\d+\b', ' ', title)
...
def _hv_is_repair_job(obj: dict) -> tuple[bool, str]:
    ...
    if all(path.startswith((".ralph-add/", "docs/audit/")) or path.endswith(".md") for path in allowed):
...
def _hv_materialize_plan(plan: dict) -> tuple[List[Path], List[Path]]:
    ...
    for raw in blockers:
        ...
        if ".ralph-add" in blocker_text and "checkout" in blocker_text:
            print("[HV] approval skipped: self-owned .ralph-add checkout noise")
            continue
...
- Do NOT return audits, inventories, reports, maps, READ_ONLY jobs, VERIFY-only jobs,
  documentation-only jobs, or work that only writes under .ralph-add/ or docs/audit/.
...
def main():
    p = argparse.ArgumentParser(prog="ralph-fixi")
