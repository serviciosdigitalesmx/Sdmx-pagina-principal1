...
POOL_FILE = Path(os.environ.get("HYPERVELOCITY_KEY_POOL_FILE", str(Path.home() / ".config" / "hypervelocity" / "provider-pool.json"))).expanduser()
...
def list_files(root: Path, raw: str) -> dict:
    ...
    for child in sorted(path.iterdir()):
        ...
        if child.name in {".git", ".hypervelocity", "node_modules", ".next", "dist", "build", "coverage"}:
            continue
        if is_secret_path(rel):
            continue
...
def key_services() -> list[str]:
    ...
    try:
        ...
        pass
    return [f"hypervelocity-gemini-{index:02d}" for index in range(1, 8)]
