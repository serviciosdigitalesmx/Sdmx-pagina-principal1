...
export function imprimirDetalle(): void {
  window.print();
}

export async function buscar(): Promise<void> {
  ...
  try {
    const data = await fixService.request<any>('get_equipo', { folio }, 'POST');
    const eq = data.equipo || data;
    if (!eq) throw new Error('No encontrado');
...
function bindEvents(): void {
  ...
  // Export search function to window for global access (legacy HTML support)
  (window as any).buscar = buscar;
  (window as any).volver = volver;
  (window as any).imprimirDetalle = imprimirDetalle;
}
