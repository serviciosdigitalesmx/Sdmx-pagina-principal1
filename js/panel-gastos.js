...
(function () {
    ...
    async function cargarResumen() {
        try {
            const data = await fixService.request('resumen_gastos', { sucursalId: getSucursalActiva(), ...getFiltros() }, 'POST');
            renderResumen(data);
        }
        catch (error) {
    ...
    async function cargarGastos({ append = false } = {}) {
        ...
        try {
            const data = await fixService.request('listar_gastos', { sucursalId: getSucursalActiva(), page: currentPage, pageSize: PAGE_SIZE, ...getFiltros() }, 'POST');
    ...
    async function eliminarGasto(id) {
        if (!confirm('¿Eliminar este gasto?'))
            return;
        try {
            await fixService.request('eliminar_gasto', { id }, 'POST');
            await cargarGastos({ append: false });
        }
        catch (error) {
