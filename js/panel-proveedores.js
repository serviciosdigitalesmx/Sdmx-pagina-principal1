...
(function () {
    ...
    async function cargarProveedores({ append = false } = {}) {
        ...
        try {
            const data = await fixService.request('listar_proveedores', payload, 'POST');
    ...
    async function guardarProveedor(ev) {
        ...
        try {
            await fixService.request('guardar_proveedor', payload, 'POST');
            cerrarModal();
    ...
    async function verDetalle(id) {
        try {
            const data = await fixService.request('proveedor', { id }, 'POST');
    ...
    async function eliminarProveedor(id) {
        if (!confirm('¿Marcar proveedor como inactivo?'))
            return;
        try {
            await fixService.request('eliminar_proveedor', { id }, 'POST');
            await cargarProveedores({ append: false });
        }
        catch (error) {
