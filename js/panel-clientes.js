...
async function cargarClientes({ append = false } = {}) {
    ...
    try {
        const data = await fixService.request('listar_clientes', { page: currentPage, pageSize: PAGE_SIZE, texto: getFiltros().texto }, 'POST');
...
async function verDetalle(id) {
    try {
        const data = await fixService.request('cliente', { id }, 'POST');
