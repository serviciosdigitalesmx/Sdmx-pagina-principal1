...
async function archivarCotizacionActual() {
    ...
    try {
        const response = await fixService.request('archivar_cotizacion', {
...
async function cargarSolicitudes(force = false) {
    ...
    elEmpty.classList.add('hidden');
    try {
        const response = await fixService.request('listar_solicitudes', {}, 'GET');
...
async function archivarSolicitud(folio) {
    try {
        const response = await fixService.request('archivar_solicitud', { folio }, 'POST');
        if (response.success === false)
