...
(function () {
    ...
    async function cargarTareas({ append = false } = {}) {
        ...
        try {
            const data = await fixService.listarTareas(payload);
            if (!data) {
    ...
    async function abrirEdicion(folio) {
        try {
            const data = await fixService.obtenerTarea(folio, getSucursalActiva());
    ...
    async function guardarTarea(ev) {
        ...
        try {
            const response = folio
                ? await fixService.actualizarTarea(payload)
                : await fixService.crearTarea(payload);
            if (response.error)
