...
export default function TenantLandingClient({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError("");
      try {
        const response = await apiClient.get<TenantData>(`/api/public/tenants/${slug}`);
        if (!response.success || !response.data) throw new Error(getApiErrorMessage(response.error, "Tenant no encontrado"));
        ...
        <div className="rounded-[36px] border border-slate-200 bg-white p-6 shadow-[0_22px_55px_rgba(15,23,42,.08)] md:p-10">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
            <div className="flex-1">
              ...
              <h1 className="mt-6 text-4xl font-semibold tracking-[-0.06em] text-slate-700 md:text-6xl">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <a href={portalHref} className="inline-flex items-center gap-2 rounded-full bg-[#121826] px-6 py-3 text-sm font-semibold text-white">
            ...
            <div className="w-full rounded-[30px] border border-slate-200 bg-[#fbfbfc] p-6 lg:w-[420px]">
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_.95fr]">
            <form onSubmit={submitRequest} className="rounded-[30px] border border-slate-200 bg-white p-6 md:p-8">
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                ...
                <textarea className="srf-input min-h-[120px] py-4 md:col-span-2" placeholder="Describe la falla reportada" value={reportedIssue} onChange={(e) => setReportedIssue(e.target.value)} required />
              ...
              <button disabled={submitting} className="mt-6 inline-flex items-center gap-2 px-6 py-4 font-black srf-btn-primary">
              ...
              {result && (
                <div className="rounded-[30px] border border-slate-200 bg-white p-6">
                  ...
                  <div className="mt-5 flex flex-wrap gap-3">
                    <a href={result.portalUrl} className="inline-flex items-center gap-2 rounded-full bg-[#121826] px-5 py-3 text-sm font-semibold text-white">
                      Ir al folio
                      <ArrowRight className="h-4 w-4" />
                    </a>
                    <button
