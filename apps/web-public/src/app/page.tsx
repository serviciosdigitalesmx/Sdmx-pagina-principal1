...
const featureCards = [
  { title: "Órdenes", description: "Crea, asigna y da seguimiento a cada reparación con el flujo del taller.", icon: "⌁" },
  { title: "Clientes", description: "Historial completo para atender cada equipo sin perder contexto.", icon: "◌" },
  { title: "Inventario", description: "Control de refacciones, accesorios y consumibles en tiempo real.", icon: "▣" },
  { title: "Evidencias", description: "Fotos y documentos por cada etapa del servicio.", icon: "◈" },
  { title: "WhatsApp", description: "Notificaciones y enlaces directos al portal del cliente.", icon: "✆" },
  { title: "Cobros y pagos", description: "Seguimiento de ingresos, comprobantes y estados de pago.", icon: "$" },
];
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p className="text-[11px] font-semibold uppercase tracking-[0.35em] text-amber-300/90">{children}</p>;
}

function CTA({ href, children, variant = "primary" }: { href: string; children: React.ReactNode; variant?: "primary" | "secondary" }) {
  const base = "inline-flex items-center justify-center rounded-2xl px-6 py-4 text-center text-sm font-semibold uppercase tracking-[0.16em] transition duration-200";
  const className =
    variant === "primary"
        ...
        <div className="rounded-[1.6rem] bg-[radial-gradient(circle_at_top,_rgba(245,158,11,0.18),_transparent_45%),linear-gradient(180deg,#111111_0%,#050505_100%)] p-5 text-white shadow-[0_18px_45px_rgba(0,0,0,0.25)]">
          ...
          <div className="mt-5 rounded-[1.2rem] border border-white/10 bg-white/5 p-4">
            <div className="grid gap-3 text-sm text-slate-200 sm:grid-cols-2">
              <div><p className="text-xs uppercase tracking-[0.2em] text-slate-400">Cliente</p><p className="mt-1 font-semibold text-white">Seguimiento real</p></div>
              <div><p className="text-xs uppercase tracking-[0.2em] text-slate-400">Equipo</p><p className="mt-1 font-semibold text-white">Dispositivo ingresado</p></div>
              <div><p className="text-xs uppercase tracking-[0.2em] text-slate-400">Estado</p><p className="mt-1 font-semibold text-white">Taller sincronizado</p></div>
              <div><p className="text-xs uppercase tracking-[0.2em] text-slate-400">Canal</p><p className="mt-1 font-semibold text-white">WhatsApp + portal</p></div>
            </div>
          </div>
          <div className="mt-4 grid grid-cols-3 gap-3 text-xs text-slate-200">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-3"><p className="uppercase tracking-[0.2em] text-slate-400">Recepción</p><p className="mt-2 text-lg font-black text-white">1</p></div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-3"><p className="uppercase tracking-[0.2em] text-slate-400">Diagnóstico</p><p className="mt-2 text-lg font-black text-white">2</p></div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-3"><p className="uppercase tracking-[0.2em] text-slate-400">Entrega</p><p className="mt-2 text-lg font-black text-white">3</p></div>
          </div>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-[1.4rem] border border-white/10 bg-white/5 p-4"><p className="text-xs uppercase tracking-[0.24em] text-amber-300">Seguimiento</p><p className="mt-2 text-sm leading-6 text-zinc-300">Portal del cliente, timeline y PDF real en un solo flujo.</p></div>
          <div className="rounded-[1.4rem] border border-white/10 bg-white/5 p-4"><p className="text-xs uppercase tracking-[0.24em] text-amber-300">Inventario</p><p className="mt-2 text-sm leading-6 text-zinc-300">Refacciones, alertas y entradas ligadas al tenant.</p></div>
          <div className="rounded-[1.4rem] border border-white/10 bg-white/5 p-4"><p className="text-xs uppercase tracking-[0.24em] text-amber-300">Cobros</p><p className="mt-2 text-sm leading-6 text-zinc-300">Mercado Pago, webhook y estados en Supabase.</p></div>
          <div className="rounded-[1.4rem] border border-white/10 bg-white/5 p-4"><p className="text-xs uppercase tracking-[0.24em] text-amber-300">WhatsApp</p><p className="mt-2 text-sm leading-6 text-zinc-300">El enlace lleva al portal con el folio precargado.</p></div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="mx-auto w-full max-w-[20rem] rounded-[2.4rem] border-[10px] border-black bg-black shadow-[0_30px_80px_rgba(0,0,0,0.32)]">
      <div className="rounded-[1.9rem] bg-[linear-gradient(180deg,rgba(16,14,12,0.98),rgba(22,18,14,0.98))] p-4 text-zinc-100">
        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-3"><p className="text-xs uppercase tracking-[0.2em] text-zinc-400">Folio</p><p className="mt-1 font-mono text-sm font-semibold text-zinc-100">FIX-00214</p></div>
        <div className="mt-3 rounded-2xl border border-white/10 bg-white/5 p-3"><p className="text-xs uppercase tracking-[0.2em] text-zinc-400">Estado</p><p className="mt-1 text-sm font-semibold text-emerald-300">En reparación</p></div>
        ...
        <button className="mt-4 w-full rounded-2xl bg-[linear-gradient(135deg,#f59e0b_0%,#fb923c_100%)] px-4 py-3 text-sm font-semibold text-zinc-950 shadow-[0_16px_35px_rgba(245,158,11,0.2)]">Consultar</button>
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(245,158,11,0.09),_transparent_26%),linear-gradient(180deg,#050505_0%,#0f0f10_46%,#141210_100%)] text-zinc-100">
      ...
      <section className="mx-auto grid w-full max-w-7xl gap-8 px-4 pb-12 pt-2 sm:px-6 lg:grid-cols-[1.03fr_0.97fr] lg:px-8 lg:pt-8">
        <div className="space-y-8 pt-4 lg:pt-10">
          ...
          <ul className="grid gap-3 text-zinc-200 sm:grid-cols-2">
            {["Clientes", "Reparaciones", "Inventario", "Cobros"].map((item) => (
          ...
          <div className="flex flex-wrap gap-6 text-sm text-zinc-400">
            {["Sin tarjeta de crédito", "Sin instalación", "Listo en minutos", "Soporte real por WhatsApp"].map((item) => (
        ...
        <div className="rounded-[2.6rem] border border-white/10 bg-[linear-gradient(180deg,rgba(16,14,12,0.96),rgba(22,18,14,0.98))] px-5 py-8 text-white shadow-[0_30px_120px_rgba(0,0,0,0.26)] sm:px-8 lg:px-10">
          <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            <div className="space-y-6">
              ...
              <h3 className="max-w-lg text-4xl font-black tracking-tight sm:text-5xl">Potente, simple y hecho para tu día a día.</h3>
              <p className="max-w-lg text-base leading-8 text-zinc-300">Lo que necesitas para operar tu taller de reparación de celulares desde cualquier dispositivo, con la operación real conectada al backend.</p>
              <ul className="space-y-3 text-sm text-zinc-200">
                {["Interfaz intuitiva y rápida", "Funciona en cualquier dispositivo", "Tus datos siempre seguros", "Actualizaciones continuas"].map((item) => (
      ...
      <section id="portal" className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
          <div className="space-y-5 rounded-[2.2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(16,14,12,0.96),rgba(22,18,14,0.98))] p-6 shadow-[0_20px_70px_rgba(0,0,0,0.24)] lg:p-8">
            ...
            <p className="max-w-xl text-base leading-8 text-zinc-300">El cliente consulta el estatus de su reparación con su número de folio, sin registro y sin apps extra. El portal usa datos reales del backend.</p>
            <ul className="space-y-3 text-zinc-200">
              {["Consulta en tiempo real", "Fotos de cada etapa", "Notificaciones automáticas", "Más confianza, más clientes"].map((item) => (
                <li key={item} className="flex items-center gap-3">
                  <span className="text-amber-300">◌</span>
                  <span>{item}</span>
                </li>
      ...
      <section id="precios" className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <p className="max-w-xl text-sm leading-7 text-zinc-300">Los planes abajo reflejan la estructura operativa disponible en la plataforma. El cobro real pasa por Mercado Pago y el backend confirma el estado.</p>
        </div>
        <div className="mt-8 grid gap-5 xl:grid-cols-3">
          {pricingPlans.map((plan) => (
            ...
            >
              {plan.featured ? <span className="inline-flex rounded-full bg-amber-500/15 px-3 py-1 text-[10px] font-black uppercase tracking-[0.28em] text-amber-200">Más popular</span> : null}
              ...
              <CTA href="/onboarding" variant={plan.featured ? "primary" : "secondary"}>Comenzar ahora</CTA>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(16,14,12,0.96),rgba(22,18,14,0.98))] px-6 py-7 text-white shadow-[0_24px_80px_rgba(0,0,0,0.24)] lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-3xl font-black tracking-tight sm:text-4xl">{productName} te deja operar con control real, sin seguir improvisando.</p>
          </div>
