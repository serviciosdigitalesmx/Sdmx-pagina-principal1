...
export function QuickReceiveModal({ open, onOpenChange }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="border-slate-800 bg-slate-950 sm:max-w-[1000px] p-0 overflow-hidden [&>button]:right-6 [&>button]:top-6">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-xs font-medium text-slate-300">Teléfono (10 dígitos) <span className="text-rose-400">*</span></label>
              <Input
                name="clientPhone"
                value={formData.clientPhone}
                onChange={handleChange}
                placeholder="Ej. 5512345678"
                className="border-slate-800 bg-slate-900 h-9 text-sm"
              />
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-4 md:border-l md:border-slate-800 md:pl-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-2">
                <label className="text-xs font-medium text-slate-300">Tipo <span className="text-rose-400">*</span></label>
                <select
                  name="deviceType"
                  value={formData.deviceType}
                  onChange={handleChange}
                  className="flex h-9 w-full rounded-md border border-slate-800 bg-slate-900 px-3 py-1 text-sm text-slate-50 shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-sky-500"
                >
              ...
              <div className="space-y-2">
                <label className="text-xs font-medium text-slate-300">IMEI/Serie</label>
                <Input
                  name="serialNumber"
            ...
            <div className="space-y-2">
              <label className="text-xs font-medium text-slate-300">Marca y Modelo <span className="text-rose-400">*</span></label>
              <Input
                name="deviceModel"
            ...
            <div className="space-y-2">
              <label className="text-xs font-medium text-slate-300">Problema reportado <span className="text-rose-400">*</span></label>
              <Textarea
                name="issue"
                value={formData.issue}
                onChange={handleChange}
                placeholder="Ej. No enciende, pantalla rota..."
                className="border-slate-800 bg-slate-900 min-h-[80px] text-sm resize-none"
              />
            </div>
          </div>
          ...
          <div className="space-y-4 md:border-l md:border-slate-800 md:pl-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-2">
                <label className="text-xs font-medium text-slate-300">Costo total ($)</label>
                <Input
                  type="number"
                  name="estimatedCost"
              ...
              <div className="space-y-2">
                <label className="text-xs font-medium text-slate-300">Abono inicial ($)</label>
                <Input
                  type="number"
                  name="deposit"
                  ...
                  onChange={handleChange}
                  placeholder="0.00"
                  className="border-slate-800 bg-slate-900 h-9 text-sm text-emerald-400 font-medium"
                />
              </div>
            </div>
            <div className="space-y-2 mt-4">
              <label className="text-xs font-medium text-slate-300">Notas internas</label>
              <Textarea
                name="notes"
                value={formData.notes}
                onChange={handleChange}
                placeholder="Falta tapa trasera, ralladuras..."
                className="border-slate-800 bg-slate-900 min-h-[60px] text-sm resize-none"
              />
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900/50 border-t border-slate-800 p-4 flex justify-end gap-3">
          <Button variant="ghost" onClick={() => onOpenChange(false)} disabled={loading} className="text-slate-400 hover:text-slate-50">
            Cancelar
          </Button>
