...
export async function runUsuariosAudit(runId: string): Promise<QaTestResult[]> {
  const config = loadQaConfig();
  const session = await createBrowserSession(runId, "usuarios");
  try {
    await session.page.goto(`${config.baseUrls.admin}/dashboard/usuarios`, { waitUntil: "networkidle" });
    let visible = true;
    try {
      await session.page.getByTestId("users-table").waitFor({ state: "visible", timeout: 30000 });
    } catch {
      visible = false;
    }
    const screenshot = join(config.screenshotsDir, runId, visualArtifactName("usuarios"));
    await session.page.screenshot({ path: screenshot, fullPage: true });
    const baseline = join(config.desktopDir, "fixtures", "baselines", visualArtifactName("usuarios"));
    const diffPath = join(config.screenshotsDir, runId, visualArtifactName("usuarios-diff"));
    ...
    return [{
      ...
      steps: [{ name: "Cargar usuarios", status: visible ? "passed" : "warning", durationMs: 0 }],
      issues: [],
