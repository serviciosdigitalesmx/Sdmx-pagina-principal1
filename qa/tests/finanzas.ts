...
export async function runFinanzasAudit(runId: string): Promise<QaTestResult[]> {
  const config = loadQaConfig();
  const session = await createBrowserSession(runId, "finanzas");
  try {
    await session.page.goto(`${config.baseUrls.admin}/dashboard/finanzas`, { waitUntil: "networkidle" });
    let visible = true;
    try {
      await session.page.getByTestId("finance-cashflow").waitFor({ state: "visible", timeout: 30000 });
    } catch {
      visible = false;
    }
    const screenshot = join(config.screenshotsDir, runId, visualArtifactName("finanzas"));
    await session.page.screenshot({ path: screenshot, fullPage: true });
    const baseline = join(config.desktopDir, "fixtures", "baselines", visualArtifactName("finanzas"));
    const diffPath = join(config.screenshotsDir, runId, visualArtifactName("finanzas-diff"));
    ...
    return [{
      ...
      steps: [{ name: "Cargar finanzas", status: visible ? "passed" : "warning", durationMs: 0 }],
      issues: [],
